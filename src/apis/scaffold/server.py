...
from services.settings import SCHEDULER_SETTINGS
...
class SystemService:
    ...
    def calibrate(self, **optional):
        ...
        scheduler_settings["decoupler"]["interval"] = max(3600, scheduler_settings["decoupler"].get("interval", 3600))
    ...
    def service_flask(self):
        raise ImportWarning

    def service_synergy(self):
        raise ImportWarning
    ...
    def startup(self):
        ...
        try:
            ...
            if self.enable_flask:
                process_list.append(
                    multiprocessing.Process(
                        target=self.service_flask,
                        name=f"{self.ACTION_NAME}|Flask"
                    )
                )
